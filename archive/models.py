...
class Document(models.Model):
    ...
    convent_type = models.CharField(
        max_length=10, choices=CONVENT_TYPE_CHOICES, null=True, blank=True
    )
    ...
    class Meta:
        db_table = 'documents'
        # Use the existing table; Django will not create or alter it.
        # Run migrations only for Django's own auth/session tables.
        managed = False
        ordering = ['-version_date', '-convent_number']

    def __str__(self):
        return f'{self.doc_type} {self.version_date}'
